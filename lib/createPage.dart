...
class _createPageState extends State<createPage> {
            ...
            task.onComplete.then((value){
              ...
              downloadUrl.then((uri){
                ...
                doc.setData({ //map을 넣는 쉬운 방법은 json형태로 넣으면됨. json 형태로 넣으면 map으로 변환되어들어. setDate는 future<void> 형태로 리턴됨 > then 사용하기 
                  ...
                  'contents' : textEditingController.text, //게시물 내
                  'email' : widget.user.email, //user 정보
                  'displayName' : widget.user.displayName,//user 정보
                  'userPhoto' : widget.user.photoUrl,//user 정보
                }).then((onValue){
 ...
 Widget _buildBody() {
  ...
  Future<void> _getImage() async {
    File image = await ImagePicker.pickImage(source: ImageSource.gallery); //Future<File> : 미래에 file이라는 형태가 될 애라는 의
